...
extern qboolean g_set_g2_player_model_info(gentity_t* pEnt, const char* modelName, const char* customSkin,
...
void G_DriveATST(gentity_t* pEnt, gentity_t* atst)
{
	...
	else
	{
		...
		else
		{
			...
			pEnt->playerModel = 0;
			g_set_g2_player_model_info(pEnt, "atst", nullptr, nullptr, nullptr);
	...
	if (pUmcd->buttons & BUTTON_USE && pVeh->m_pVehicleInfo->type != VH_SPEEDER)
	{
		...
		else if (!(pVeh->m_ulFlags & VEH_FLYING))
		{
			...
			if (parent->client->ps.speed <= 600 && pUmcd->rightmove != 0)
			{
				if (pVeh->m_pVehicleInfo->Eject(pVeh, pRider, qfalse))
				{
					constexpr int i_flags = SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_HOLDLESS, i_blend = 300;
