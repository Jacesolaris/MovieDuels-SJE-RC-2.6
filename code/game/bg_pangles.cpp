...
void BG_IK_MoveLimb(CGhoul2Info_v& ghoul2, const int bolt_index, const char* anim_bone, const char* first_bone, const char* second_bone,
	...
	vec3_t angles, vec3_t scale, const int blend_time, const qboolean force_halt)
{
	...
	else if (*ik_in_progress)
	{
		...
		gi.G2API_GetBoneAnim(&ghoul2[0], anim_bone, static_cast<const int>(time), &c_frame, &s_frame, &e_frame, &flags,
			&anim_speed, nullptr);
