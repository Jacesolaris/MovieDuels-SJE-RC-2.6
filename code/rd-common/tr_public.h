...
typedef struct {
	...
	void	(*Font_DrawString)(int x, int y, const char* s, const float* rgba, int iFontHandle, int iMaxPixelWidth, float scale);
	...
	qboolean(*G2API_GetBoltMatrix)(CGhoul2Info_v& ghoul2, int modelIndex, int boltIndex, mdxaBone_t* matrix,
		const vec3_t angles, const vec3_t position, int AframeNum, qhandle_t* modelList, const vec3_t scale);
	...
	qboolean(*G2API_SetBoneAnglesIndex)(CGhoul2Info* ghlInfo, int index, const vec3_t angles, int flags,
		Eorientations yaw, Eorientations pitch, Eorientations roll, qhandle_t* modelList,
		int blendTime, int AcurrentTime);
