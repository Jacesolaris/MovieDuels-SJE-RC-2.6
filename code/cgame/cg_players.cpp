...
void CG_AddGhoul2Mark(const int type, const float size, vec3_t hitloc, vec3_t hitdirection,
	const int entnum, vec3_t entposition, const float entangle, CGhoul2Info_v& ghoul2, vec3_t model_scale,
...
static void CG_RegisterCustomSounds(clientInfo_t* ci, const int i_sound_entry_base, const int i_table_entries, const char* pps_table[], const char* ps_dir)
{
...
qboolean CG_TryPlayCustomSound(vec3_t origin, const int entity_num, const soundChannel_t channel, const char* sound_name, const int custom_sound_set)
{
	...
	case AEV_EFFECT:
		if (anim_event->eventData[AED_EFFECTINDEX] == -1)
		{
				...
				&& anim_event->stringData[0])
			{
				...
				{
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.client_num, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/push_md.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.client_num, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/push.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.client_num, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/pushlow.mp3"));
					cgi_S_StartSound(cent->lerpOrigin, cent->currentState.client_num, CHAN_AUTO, cgi_S_RegisterSound("sound/weapons/force/pushhard.mp3"));
					cent->gent->client->ps.powerups[PW_FORCE_PUSH_RHAND] = cg.time + anim_event->eventData[AED_EFFECT_PROBABILITY];
					//AED_EFFECT_PROBABILITY in this case is the number of ms for the effect to last
					cent->gent->client->pushEffectFadeTime = 0;
				}
				else if (Q_stricmp("scepter_beam", anim_event->stringData) == 0)
				{
					...
					if (model_index > 0)
					{
						...
						if (bolt_index > -1)
						{
							...
							CG_PlayEffectBolted("scepter/beam.efx", model_index, bolt_index, cent->currentState.client_num, cent->lerpOrigin, anim_event->eventData[AED_EFFECT_PROBABILITY], qtrue);
						}
					}
				}
				//FIXME: add more
	...
	vec3_t thoracic_angles, vec3_t head_clamp_min_angles, vec3_t head_clamp_max_angles)
{
	...
	else
	{
		...
		BG_G2SetBoneAngles(cent, cent->gent->cervicalBone, neck_angles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
			NEGATIVE_Z, cgs.model_draw);
		BG_G2SetBoneAngles(cent, cent->gent->thoracicBone, thoracic_angles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
			NEGATIVE_Z, cgs.model_draw);
	}
}

static void CG_UpdateLookAngles(const centity_t* cent, vec3_t look_angles, const float look_speed, const float min_pitch, const float max_pitch,
...
static void CG_G2PlayerAngles(centity_t* cent, vec3_t legs[], vec3_t angles)
{
		...
		&& (cent->gent->client->NPC_class != CLASS_ATST || !cent->gent->s.number))
	{
		...
		if (cent->gent && cent->gent->client && cent->gent->client->NPC_class == CLASS_ATST)
		{
			...
			BG_G2SetBoneAngles(cent, cent->gent->thoracicBone, look_angles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
				NEGATIVE_Z, cgs.model_draw);
		}
		else
		{
...
static void CG_ForceElectrocution(const centity_t* cent, const vec3_t origin, vec3_t temp_angles, const qhandle_t shader, const qboolean always_do = qfalse)
{
...
void CG_AddRefEntityWithPowerups(refEntity_t* ent, int powerups, centity_t* cent)
{
	...
	*/
	...
	{
		if (cg_SaberInnonblockableAttackWarning.integer == 1 || d_SaberactionInfo->integer || cg_DebugSaberCombat.integer)
		{
...
void CG_SaberDoWeaponHitMarks(const gclient_t* client, const gentity_t* saber_ent, gentity_t* hit_ent, const int saber_num, const int blade_num, vec3_t
...
//Movie Sabers
void CG_DoEp1Saber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
...
void CG_DoEp2Saber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
void CG_DoEp3Saber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
void CG_DoSFXSaber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
void CG_DoOTSaber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
void CG_DoRotJSaber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
void CG_DoTFASaber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
static void CG_DoCloakedSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color, int rfx, qboolean do_light)
{
	vec3_t mid;
...
static void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color, int rfx, qboolean do_light)
{
	vec3_t mid;
	qhandle_t blade = 0, glow = 0;
	refEntity_t saber;
...
void CG_DoUnstableSaber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
	float blade_len, end_len, trail_len, base_len, dis_tip, dis_muz, dis_dif;
...
static void CG_DoSaberUnstable(vec3_t origin, vec3_t dir, float length, float length_max, float radius, saber_colors_t color, int rfx, qboolean do_light)
{
	vec3_t mid;
	qhandle_t blade = 0, glow = 0;
	refEntity_t saber;
...
void CG_DoRebelsSaber(centity_t* cent, vec3_t blade_muz, vec3_t blade_tip, vec3_t trail_tip, vec3_t trail_muz, float length_max, float radius, saber_colors_t
	color, int rfx, qboolean do_light)
{
	vec3_t dif, mid, blade_dir, end_dir, trail_dir, base_dir;
	float radiusmult, effectradius, coreradius, effectalpha, angle_scale;
...
static void CG_AddSaberBladeGo(centity_t* cent, centity_t* scent, const int renderfx, const int model_index, vec3_t origin, vec3_t angles, const int saber_num, const int blade_num)
{
					...
					if (!no_marks)
					{
						...
						if (client->ps.saber[saber_num].blade[blade_num].trail.haveOldPos[i])
						{
							...
							else if (!i)
							{
								...
								CG_SaberDoWeaponHitMarks(client, scent != nullptr ? scent->gent : nullptr, hit_ent, saber_num, blade_num, trace.endpos, axis[0], uaxis, 0.25f);
							}
						}
						else
						{
...
void CG_AddSaberBlade(centity_t* cent, centity_t* scent, const int renderfx, const int modelIndex, vec3_t origin, vec3_t angles)
{
